...
| M5 Cash/share simulator | `complete` | [M5 證據](evidence/m5-cash-share-ledger-2026-08-19.md)、canonical `tw_sepa_screener.ledger`、鏡像 `m5/`、**51 passed** | 帳本恆等式與不可能交易阻擋通過 | M0 §6 六項不變量全部以測試強制；`test_no_impossible_trade_can_settle` 由 strict xfail 轉為實測。source-state 指紋 `898ef48a…` → `9820fd43…`。**下一步 M6 adapter**：把 as-of 重建逐日餵進帳本 |
