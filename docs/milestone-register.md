...
| M6 AlphaMaster adapter | `in_progress` | [Phase 0 證據](evidence/m6-phase0-cost-recompute-2026-08-20.md)、`scripts/m6/phase0_cost_recompute.py` | frozen dataset 可重現研究 run | **Phase 0 完成**：既有 SEPA 回測在一百萬資金下成本模型誤差近乎為零，但依 M0 風險政策縮至一萬元後成本由 0.60% 升至 4.60%，三個情境有兩個由正轉負。剩餘 Phase 2/3（as-of 股票池、M5 帳本成交）屬基礎建設；M3 窗僅 382 個交易日，同策略約僅 10 筆交易，不足以下策略結論 |
