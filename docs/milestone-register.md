...
| M3 Point-in-time warehouse | `in_progress`（M3.2 起 `blocked`）| [M3 計畫](m3-point-in-time-warehouse-plan.md)、[PIT 契約](contracts/pit-warehouse-contract.md)、[source-to-table map 與 policy](contracts/m3-source-to-table-map.md)、[G0 Owner 決定](evidence/m3-g0-owner-decision-2026-08-03.md)、[entry baseline](evidence/m3-entry-baseline-2026-08-03.md)、[M3.1 完成證據](evidence/m3-1-coverage-ledger-and-durable-archival-2026-08-16.md) | 2025-01-01 至 2026-08-03 每個 TWSE／TPEx calendar date 都有 certificate；開市／特殊 session 全為 `supported`、休市日有官方 `not-session`；未列或 partial／blocked／unknown 皆 fail closed | M3.0／M3.1 已 `complete`。1,160 列 ledger 顯示 `supported`＝0、缺 1,143 個 market-date。M3.2 需 Owner 先決定歷史抓取程式、TPEx 日曆來源或調整 G0 期間 |
...
- M3.1 `complete` 不代表已有可重建的歷史；1,160 列 coverage ledger 中 `supported` 為 0，2025 全年沒有任何官方歷史證據。
