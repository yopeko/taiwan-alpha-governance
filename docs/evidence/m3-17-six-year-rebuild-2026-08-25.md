...
### 4.1 同一個形狀的第三到第六例，其中一例是驗證器自己

修完那五個測試檔之後，`validate_m3_7.py` 跑出 `verdict: passed`——**7.3 分鐘**，而我估的是 160 分鐘。那個落差就是線索：

| | 它驗的 | 六年表 |
|---|---|---|
| staging dataset_id | `c05b4b54…` | `ae98a5d4…` |
| price_rows | 828,736 | **3,316,101** |

它自己寫死了 `staging-10`、`pit-04`、`pit-prices-09`。**剛修完的同一個缺陷，在一支不在 `tests/` 底下的檔案裡，所以沒被看到。**

因此把規則一般化為 [`test_no_hardcoded_warehouse_paths.py`](../../tests/unit/test_no_hardcoded_warehouse_paths.py)：**除了單一來源，任何模組都不得寫死倉庫路徑**。它立刻又抓出三個：

| 模組 | 後果 |
|---|---|
| `scripts/m3/asof.py` | `default_warehouse()` 指向舊倉庫——**as-of 重建介面本身**，所有取預設值的呼叫端都會從上一代回答問題，而且成功回答 |
| `scripts/audit/crossvalidate_finmind.py` | 那個「用獨立來源問倉庫它問不了自己的問題」的稽核，會去驗上一代並正確地判定它完整 |
| `scripts/audit/crossvalidate_trading_status.py` | 兩個 argparse 預設值——而預設值是幾乎每次執行都會用到的東西 |

單一來源移至 [`scripts/m3/current_build.py`](../../scripts/m3/current_build.py)，因為世代是 build 的屬性，而驗證器與稽核腳本也要讀它；`tests/warehouse.py` 改為轉引。

**同一形狀共六例**：五個測試檔、驗證器、as-of 介面、兩支稽核。失敗都是靜默的，輸出都是綠的。世代是加性的——舊目錄留在磁碟上讓舊報告仍可重讀，這正是釘住的路徑會繼續運作、也繼續說謊的原因。
...
|---|---|
| staging 決定性 | passed |
| 表決定性 | passed，`trading_calendar_pit` sha `c83e476088fb9fee` **與 pit-12 manifest 逐位元組相同** |
| restore drill | 163 檔相同 |
| 受保護 stores | 未變動 |
| legacy 比對 | compared |

price_rows 3,316,101、status_rows 55,030，與六年表相符——這次驗的是對的東西。

**legacy 探針由 `[0,0,0]` 變為 `[1709,0,0]`。** 探針是從 PIT 表推導的（首、中、末），六年表把第一個由 2025-01-02 換成 2019-01-02，而 legacy 在該日有 0 列。報告的說明字串宣稱「非零差異由範圍解釋」——那是斷言不是檢查，所以實查：**legacy `daily_prices` 涵蓋 2020-08-03 至 2026-08-25**。2019-01-02 不在其中，是範圍不是分歧；中位探針 2022-10-13 為 1,777/1,777 完全相符。

順帶得到一個正面結果：**PIT 倉庫現在比正式系統往回多了 19 個月。**

## 5. D15（已裁決：選項 A）：公告日 lane 沒有跟著回補
...
**Owner 於 2026-08-25 裁決採 A，並於同日提供匯出完成匯入**（[決定書](m3-owner-decisions-d14-d15-2026-08-25.md)）。

7,947 + 4,635 列、拒絕 0，lane 由 5,010 增至 **17,592 列**，`ex_date` 涵蓋 2019-01-16 至 2026-08-18。**未改任何程式**——`tej_dividend_announcements()` rglob 整條 lane 的每個 snapshot。

重建後 `first-observed-only` 由 **5,292 降至 530**，不可用比例 **77.2% → 4.2%**，且逐年落在 2.8%–7.2% 之間——**斷崖消失**。

剩餘 530 筆中 446 筆（84%）是 `ex_rights`（純除權／現金增資配股），這是**既有限制**：涵蓋最好的 2025 年也是 0% publisher-exact，因為 TEJ 的現金股利表不帶純除權。要補需另找除權／現金增資表。

### 守衛改寫：我自己寫的一個假陰性

[`test_vendor_lane_window_parity.py`](../../tests/invariant/test_vendor_lane_window_parity.py) 原本斷言「lane 最早 `ex_date` 不晚於窗口起始日」，並記為 strict xfail。

回補之後 lane 最早是 **2019-01-16**——因為 2019 年前兩週沒有任何證券除息。**lane 完整涵蓋窗口，斷言仍然失敗。**

**一個事件 lane 的最早事件，不等於它的涵蓋起點。** 改為逐年檢查：價格涵蓋的每一年都必須有廠商列。那才是缺口真正違反的性質（六年完全沒有），而且不依賴第一個事件恰好落在哪一天。strict xfail 移除，測試改為正常通過。

## 6. 本次不代表

- ~~**`validate_m3_7.py` 尚未執行。**~~ **已於 2026-08-25 通過**，但過程本身是本節第四則事件，見 §4.1。
- M6 資料集與帳本回測尚未以六年窗口重跑。
- D14 已追認，D15 已裁決採 A **且匯出已完成**，公司行動不可知比例降至 4.2%。剩餘的純除權缺口是既有限制，未由本次處理。
