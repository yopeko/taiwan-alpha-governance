...
| | 外人 clone 之後 |
|---|---|
| **957 個測試** | **769 個不需要任何外部資料，4 秒跑完。** 其餘 188 個標了 `needs_local_data`：需要作業者的倉庫與封存，在別的機器上會**印出理由後 skip**，不會假裝通過。 |
| **59 支腳本** | **0 支能跑。** 全部指向未公開的 `tw-sepa-screener` 倉庫與授權廠商資料。 |
| **106 份文件** | 全部可讀。**這是這個倉庫的主體。** |
...
**[完整索引在 docs/INDEX.md](docs/INDEX.md)**（106 份）。從這三份開始：
