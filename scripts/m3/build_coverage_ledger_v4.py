...
TPEX_ACTIONS = RAW / "m3_tpex_actions_2024-2026"
TEJ_LANE = RAW / "m3_tej_licensed_2026-08-16"
OUT_DIR = RAW / "m3_coverage_ledger_2026-08-16-v4"
...
def tpex_action_years() -> set[int]:
    ...
    ledger = TPEX_ACTIONS / "tpex_actions_ledger.jsonl"
    ...
    for line in ledger.read_text(encoding="utf-8").splitlines():
        ...
        if record.get("outcome") in {"captured", "official-no-announcements"}:
            years.add(year)
        else:
            failed.add(year)
    retry = TPEX_ACTIONS / "tpex_detail_retry.json"
    if retry.is_file():
...
def main() -> None:
    ...
    summary = {
        ...
        "certificate_id": "tw-alpha-m3-coverage-ledger-20260816-04",
